window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/* 对外口径 = 「被接管的发送次数」，不是 record 条数：
		   一次发送会同时记 keydown-enter/click-send 与 optimize-start 两条，
		   直接用 intercepts.length 会把 1 次发送显示成 2。徽标与浮层文案共用本函数。 */
		function interceptedSends() {
			let c = 0;
			for (const r of store.intercepts) { if (r.kind === "keydown-enter" || r.kind === "click-send") c++; }
			return c;
		}
		...
		function helpPopover() {
			...
			return h("div", {
				...
				h("div", { className: "dpo-help-meta", "data-dpo": "help-meta" },
					"作者：啃轮胎的西狐 · 版本 0.1.1beta3 · 版本日期 2026/09/13"),
			);
		}

window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const store = {
			...
			tierBySession: {},
			permissionBySession: {},
			/* 按会话的「已接管发送次数」。必须落盘（host 的 perSession[sid].sends），
			   否则刷新页面/重启 DSH 后归零、徽标消失。 */
			sendsBySession: {},
			helpOpen: false,
			helpPos: null,
		};
		...
		function record(kind, text, extra) {
			...
			/* 接管计数与「拦截事件」同源：只有 keydown-enter / click-send 算一次发送，
			   optimize-start 不算（否则一次发送会被算成两次）。先加计数再 emit，徽标当帧即为新值。 */
			if (kind === "keydown-enter" || kind === "click-send") bumpSends();
			emit();
			return row;
		}
		/* 徽标口径 = 「**本会话**已接管的发送次数」，三个条件都要满足：
		   ① 不是 record 条数 —— 一次发送会同时记 keydown-enter/click-send 与 optimize-start 两条，
		      用 intercepts.length 会把 1 次发送显示成 2；
		   ② 不是全局 —— 必须按当前会话分开，否则 A 会话拦过 1 次、切到 B 会话照样显示 1；
		   ③ 不能只在内存里 —— 刷新页面 / 重启 DSH 会清空内存。真正的计数存在 store.sendsBySession，
		      由 host 的 perSession[sid].sends 落盘并回灌，见 bumpSends()。
		   store.intercepts 仍保留为原始事件流水（自检探针依赖它），但不再作为徽标数据源。 */
		function interceptedSends() {
			const n = store.sendsBySession[sessionKey()];
			return typeof n === "number" && Number.isFinite(n) && n > 0 ? Math.round(n) : 0;
		}
		/** 接管 +1：写进本会话槽位并立即落盘（跨刷新 / 重启保留）。 */
		function bumpSends() {
			const next = interceptedSends() + 1;
			store.sendsBySession[sessionKey()] = next;
			persistState({ sends: next });
			return next;
		}
		...
		function Slider(props) {
			...
			/* fillPct 是"已选格数占比"，与 pct（把手位置占比）刻意分开：
			   档位条已改成纯格子填充，选到第 i 档就应该填满前 i+1 格 —— 共 n 格。
			   原来直接复用 idx/last 会让最高档只填到 (n-1)/n（4 档时 66.67%），最后一格永远空着。
			   pct 仍按 idx/last 计算，交给 CSS 做把手定位（把手已视觉隐藏，仅保留位置语义）。 */
			const n = Math.max(1, props.options.length);
			const fillPct = ((idx + 1) / n) * 100;
			const pct = (idx / last) * 100;
			/* 每格固定宽度 ⇒ 轨道总宽 = 格数 × 格宽。
			   档位条改成"格子"语义后，2 档与 4 档必须每格等大，所以总宽随档位数变化：
			   4 档 × 30px = 120px，2 档 × 30px = 60px（即审查条是等级条的一半）。
			   用内联样式给出，避免上面 v52/v53 的 @media 把宽度改成另一套数值而破坏等格。 */
			const cellW = props.cell || 30;
			const trackW = n * cellW;
			const pickFromX = (clientX) => {
				...
				/* 取档必须与"格子"同源：轨道被均分成 n 个格子，第 i 格的范围是
				   [i/n, (i+1)/n)。所以索引 = floor(t * n)，而不是 round(t * (n-1))。
				   原先用 Math.round(t * last)（last = n-1）会得到 1/6 的分界，
				   与 1/4 的格子边界错位 —— 实测点第 4 格左半（x=0.75~0.83）会被算成"高级"，
				   这正是"点最高档却选中中间档"的成因。 */
				const opt = props.options[Math.min(n - 1, Math.floor(t * n))];
			...
			return h("div", {
				...
				h("div", {
					...
					style: { flex: "0 0 " + trackW + "px", width: trackW + "px", minWidth: trackW + "px", maxWidth: trackW + "px" },
					onPointerDown: onDown, onPointerMove: onMove, onPointerUp: onUp, onPointerCancel: onUp,
				},
					h("div", { className: "dpo-slider-fill", style: { width: fillPct + "%" } }),
		...
		function helpPopover() {
			...
			return h("div", {
				...
				h("div", { className: "dpo-help-meta", "data-dpo": "help-meta" },
					"作者：啃轮胎的西狐 · 版本 0.1.1beta5 · 版本日期 2026/09/13"),
			);
		}
			...
			return h("div", {
				...
				store.narrow === true
					? h("div", { className: "dpo-pop-sliders" },
						...
						h(Slider, { name: "perm", cell: 30, title: "优化权限", options: PERMISSIONS, value: store.permission, disabled: store.tier === "off", onPick: (id) => setPermission(id) }),
					)
					: null,
		...
		function Controls(props) {
			...
			const n = interceptedSends();
			const off = store.tier === "off";
			if (!isActiveInstance()) return null; // 旧实例不再渲染控件（新实例已接管）
			return h("div", { className: "dpo-controls-wrap" },
				h("div", { ref: nodeRef, className: "dpo-controls", "data-dpo": "controls" },
				store.narrow === true
					? h("button", {
				...
				store.notice && Date.now() < store.notice.until
					...
					: null,
				modelPopover(),
				n > 0 ? h("span", { className: "dpo-count", "data-dpo": "count", title: "本会话已接管 " + n + " 次发送" }, String(n)) : null,
				),
			);
		}
		...
		/** 取消在飞运行：停掉生产者（关流 + 宿主 abort）并标记为已消费。
		    放行/确认提交会把消息交回官方链路，此后这条运行的终态不得再发送一次。
		    收尾动作只针对仍在飞的运行：已到终态的运行不再发 abort（否则宿主会把
		    一条已完成的运行改记成 aborted）。 */
		function cancelRun(run) {
			if (!run || run.released === true) return;
			run.released = true;
			const inFlight = run.status === "connecting" || run.status === "running";
			if (run.es) { try { run.es.close() } catch (e) { /* noop */ } run.es = null; }
			if (inFlight && run.runId) {
				fetch(API + "/run/abort", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ runId: run.runId }) }).catch(() => {});
			}
		}
		...
		function startRun(request, tier, forceError, opts) {
			...
			const run = {
				...
				sessionId: sid, readyToSend: null, released: false,
			};
			store.run = run;
			store.reviewText = null;
			store.regenAsk = false;
			...
			fetch(API + "/run", {
				...
				es.onmessage = (ev) => {
					...
					if (msg.type === "snapshot") {
						...
						// 已被用户放行/确认提交的运行：终态不得再发一次（放行时已取消，这里兜住"事件已在途"）
						if (cur.released === true) {
							beacon("terminal-after-release", { kind: "error", chars: String(cur.request || "").length });
						...
						cur.status = "done";
						if (cur.released === true) {
							// 用户已放行/确认提交：这条运行的产出作废，绝不再发第二次
							beacon("terminal-after-release", { kind: "done", chars: String(cur.text || "").length });
		...
		function Overlay() {
			...
			return h("div", {
				...
				h("div", { className: "dpo-overlay-scroll" },
					h("div", { className: "dpo-overlay-src" }, "档位：" + (TIERS.find((x) => x.id === store.tier) || {}).label + " · 权限：" + (PERMISSIONS.find((x) => x.id === store.permission) || {}).label + " · 拦截累计 " + interceptedSends()),
					store.rollbackConfirm === true
						? h("div", { className: "dpo-overlay-src", "data-dpo": "rollback-confirm" },
							h("span", null, "确定回退？将停止优化、关闭浮层，且不发送任何消息。"),
							h("button", { type: "button", className: "dpo-btn danger", "data-dpo": "rollback-yes", onClick: () => { beacon("rollback-yes", {}); rollbackYes(); } }, "确定回退"),
		...
		exports.apply = function apply(ctx) {
			...
			fetch(API + "/state", { cache: "no-store" }).then((r) => r.json()).then((d) => {
				...
				// 按会话的档位/权限/接管计数：先装入表（无论用户是否刚改过，都要装上）
				if (st.perSession && typeof st.perSession === "object") {
					for (const k of Object.keys(st.perSession)) {
						...
						// 接管计数回灌：徽标靠它跨刷新/重启保持，不再归零
						if (typeof v.sends === "number" && Number.isFinite(v.sends) && v.sends > 0) store.sendsBySession[k] = Math.round(v.sends);
					}
					beacon("per-session-restored", { sessions: Object.keys(st.perSession).length, current: store.viewSessionId || null, sends: interceptedSends() });
				}
			...
			own(() => {
				...
				const onKey = (e) => {
					...
					e.preventDefault();
					e.stopPropagation();
					const row = record("keydown-enter", draftLive(), { sessionId: store.viewSessionId || null });
				...
				const onClick = (e) => {
					...
					e.preventDefault();
					e.stopPropagation();
					const row = record("click-send", draftLive(), { label, sessionId: store.viewSessionId || null });
					interceptAndOptimize(row.text);
				};
			...
			own(() => {
				...
				style.textContent = [
					...
					/* ══════════ v53 主题自适应层（追加覆写：只改配色与材质，不动几何、动效与拦截逻辑） ══════════
					   前两层（基础/v51/v52）把亮色能力写死了：内高光是 rgba(255,255,255,.07)、滑块凹槽与文本阴影是
					   rgba(0,0,0,…)、强调色亮端是 62% 白、状态色是钉死的十六进制。这些在暗底上成立，但 DSH 的
					   面包屑/侧栏/壁纸一旦变亮，半透明白高光会消失、黑色阴影会发灰成"脏块"。
					   本层把每个 --dpo-* 语义变量重新指向 DSH 主题别名 token（亮暗各一套，theme 包已定义），
					   于是同一份样式在 body / body[data-ds-dark-theme] 下自动取到正确取值；token 缺失时才用回退值。 */
					".dpo-controls,.dpo-pop,.dpo-overlay{",
					/* 强调色：用宿主"按钮填充色"而不是 state-business-primary。
					   实测差别很关键：state-business-primary 在暗色主题取 deepseek-400(#679efe)，
					   白字压上去只有 2.66:1（低于 WCAG 3:1）；而 button-info-fill 暗色是 deepseek-400
					   以外的那一档、亮色 #4176e6，白字分别是 4.23:1 与 4.5:1，与宿主主按钮 .uV2eYG_primary
					   （background:var(--dsw-alias-button-info-fill);color:#fff）完全一致。 */
					"  --dpo-acc:var(--dsw-alias-button-info-fill,var(--dsw-alias-state-business-primary,#4a9eff));",
					/* 强调色亮端：只在强调色自身里提亮，不再往白里混，亮底上才不会是"漂白的一块" */
					"  --dpo-acc-2:color-mix(in srgb,var(--dpo-acc) 82%,var(--dsw-alias-label-primary,#fff));",
					/* 表面：跟随宿主浮层表面，壁纸/自定义皮肤下同样对得上 */
					"  --dpo-surface:var(--dsw-specific-tip,var(--dsw-alias-bg-layer-2,#1b1b1b));",
					"  --dpo-line:var(--dsw-alias-border-l2,var(--dsw-alias-border-l1,#3a3a3a));",
					/* 内高光/内阴影：亮底上是"深色 6%"，暗底上是"浅色 7%"，随主题翻转 */
					"  --dpo-hi:color-mix(in srgb,var(--dsw-alias-label-primary,#fff) 7%,transparent);",
					"  --dpo-lo:color-mix(in srgb,var(--dsw-alias-label-primary,#000) 12%,transparent);",
					"  --dpo-lo-soft:color-mix(in srgb,var(--dsw-alias-label-primary,#000) 6%,transparent);",
					/* 阴影：交给宿主 elevation token（含描边+柔光），不再自造重黑投影 */
					"  --dpo-shadow-1:var(--dsw-elevation-panel,0 1px 2px rgba(0,0,0,.18),0 2px 8px rgba(0,0,0,.14));",
					"  --dpo-shadow-2:var(--dsw-elevation-prominent,0 2px 6px rgba(0,0,0,.20),0 18px 48px rgba(0,0,0,.26));",
					/* 状态色：交给宿主 state token，换皮肤/变亮暗都跟着走 */
					"  --dpo-ok:var(--dsw-alias-state-success-primary,#3ecf8e);",
					"  --dpo-err:var(--dsw-alias-state-error-primary,#f2777a);",
					"  --dpo-warn:var(--dsw-alias-state-warn-primary,#e0a33e);",
					"  --dpo-err-soft:color-mix(in srgb,var(--dpo-err) 13%,transparent);",
					"  --dpo-err-line:color-mix(in srgb,var(--dpo-err) 32%,transparent);",
					"}",
					/* 滑块：凹槽、内阴影、拇指描边全部改走 token，避免亮底上出现黑色轨道 */
					".dpo-slider-track::before{background:linear-gradient(180deg,var(--dpo-lo),var(--dpo-lo-soft));box-shadow:inset 0 1px 2px color-mix(in srgb,var(--dsw-alias-label-primary,#000) 20%,transparent),0 1px 0 var(--dpo-hi)}",
					".dpo-slider-thumb{box-shadow:0 0 0 2px var(--dpo-surface),0 0 0 3px var(--dpo-acc-22),0 2px 6px var(--dpo-lo)}",
					".dpo-slider:hover .dpo-slider-thumb{box-shadow:0 0 0 2px var(--dpo-surface),0 0 0 4px var(--dpo-acc-22),0 3px 10px var(--dpo-lo)}",
					".dpo-slider-value{color:var(--dsw-alias-label-primary,#eee)}",
					".dpo-slider[data-disabled=\"true\"]{opacity:.5}",
					/* 模型/帮助胶囊：内高光与外投影改走 token */
					".dpo-model{box-shadow:var(--dpo-shadow-1)}",
					".dpo-help{box-shadow:var(--dpo-shadow-1)}",
					/* 弹层与浮层：表面不透明回退 + backdrop-filter 才半透明（@supports 天然提供回退） */
					".dpo-pop{background:var(--dpo-surface);border-color:color-mix(in srgb,var(--dpo-line) 80%,transparent)}",
					"@supports (backdrop-filter:blur(1px)){.dpo-pop{background:color-mix(in srgb,var(--dpo-surface) 88%,transparent)}}",
					".dpo-pop-item:hover{background:var(--dpo-acc-12);color:var(--dpo-acc)}",
					".dpo-pop-item[data-selected=\"true\"]:hover{background:var(--dpo-acc-12)}",
					".dpo-pop-empty{color:var(--dsw-alias-label-tertiary,#999)}",
					".dpo-overlay{border-color:color-mix(in srgb,var(--dpo-line) 85%,transparent);box-shadow:var(--dpo-shadow-2),inset 0 1px 0 var(--dpo-hi)}",
					"@supports (backdrop-filter:blur(1px)){.dpo-overlay{background:color-mix(in srgb,var(--dpo-surface) 90%,transparent)}}",
					/* 常驻底栏/头部在浮层表面上有底色时不能保持半透明，否则内容会透出来 */
					".dpo-review .dpo-overlay-actions{background:var(--dpo-surface)}",
					"@supports (backdrop-filter:blur(1px)){.dpo-review .dpo-overlay-actions{background:color-mix(in srgb,var(--dpo-surface) 92%,transparent);backdrop-filter:blur(12px) saturate(1.2)}}",
					/* 只读文本框：凹面与阴影改走 token */
					".dpo-review-text{background:color-mix(in srgb,var(--dpo-surface) 92%,transparent);box-shadow:inset 0 1px 2px var(--dpo-lo-soft)}",
					/* 面板/提示/行 hover：不再用钉死的 rgba 强调色 */
					".dpo-pane{border-color:color-mix(in srgb,var(--dpo-line) 80%,transparent);background:linear-gradient(180deg,var(--dpo-hi),transparent);box-shadow:inset 0 1px 0 var(--dpo-hi)}",
					".dpo-pane:hover{border-color:var(--dpo-acc-22);box-shadow:inset 0 1px 0 var(--dpo-hi),0 2px 12px var(--dpo-shadow-1)}",
					".dpo-pane[data-kind=\"text\"] .dpo-pane-title::before{background:var(--dpo-acc-2)}",
					".dpo-notice{background:var(--dpo-acc-12);color:var(--dpo-acc);box-shadow:0 2px 12px var(--dpo-acc-12)}",
					/* 计数徽标：与「?」按钮（.dpo-help）同款边框与填充 —— 中性表面而非强调色块，
					   避免它看起来像个能点的按钮；形状仍保持小药丸，尺寸不变。 */
					".dpo-count{background:linear-gradient(180deg,color-mix(in srgb,var(--dsw-alias-label-primary,#fff) 5%,transparent),transparent);color:var(--dsw-alias-label-secondary,#aaa);border:1px solid var(--dpo-line)}",
					/* 状态点：成功/错误/降级改用 state token */
					".dpo-overlay[data-state=\"done\"] .dpo-overlay-head::before{background:var(--dpo-ok);box-shadow:0 0 8px color-mix(in srgb,var(--dpo-ok) 50%,transparent)}",
					".dpo-overlay[data-state=\"error\"] .dpo-overlay-head::before{background:var(--dpo-err);box-shadow:0 0 8px color-mix(in srgb,var(--dpo-err) 50%,transparent)}",
					".dpo-overlay[data-dpo-degraded=\"1\"] .dpo-overlay-head::before{background:var(--dpo-warn);box-shadow:0 0 8px color-mix(in srgb,var(--dpo-warn) 50%,transparent)}",
					/* 错误态 */
					".dpo-run-error{color:var(--dpo-err);background:var(--dpo-err-soft);border:1px solid var(--dpo-err-line)}",
					/* 关闭按钮的红色 hover */
					".dpo-x:hover{background:var(--dpo-err-soft);color:var(--dpo-err);border-color:var(--dpo-err-line)}",
					/* 按钮：实心色块上的文字用**上下文前景色** token（亮色主题=白、暗色主题=深），
					   它本身就是"当前主题下压在强调色上最可读"的那一档：
					   实测亮色 #4176e6 底 + 白字 = 4.5:1；暗色 #679efe 底 + #0f1115 字 = 7.8:1。
					   若强行固定 #fff，暗色会掉到 2.66:1（低于 WCAG 3:1），所以这里必须跟随 token。 */
					".dpo-btn{color:var(--dsw-alias-label-secondary,#ccc)}",
					".dpo-btn:hover{color:var(--dpo-acc);border-color:var(--dpo-acc)}",
					".dpo-btn.primary{color:var(--dsw-alias-label-primary-foreground,#fff);box-shadow:0 2px 10px var(--dpo-acc-22),inset 0 1px 0 color-mix(in srgb,var(--dsw-alias-label-primary-foreground,#fff) 22%,transparent);background:linear-gradient(180deg,var(--dpo-acc-2),var(--dpo-acc))}",
					".dpo-btn.primary:hover{color:var(--dsw-alias-label-primary-foreground,#fff);box-shadow:0 4px 16px var(--dpo-acc-40),inset 0 1px 0 color-mix(in srgb,var(--dsw-alias-label-primary-foreground,#fff) 28%,transparent)}",
					".dpo-btn.danger{background:var(--dpo-err);border-color:transparent;color:var(--dsw-alias-label-primary-foreground,#fff);box-shadow:0 2px 10px var(--dpo-err-line),inset 0 1px 0 color-mix(in srgb,var(--dsw-alias-label-primary-foreground,#fff) 18%,transparent)}",
					".dpo-btn.danger:hover{background:var(--dpo-err);border-color:transparent;color:var(--dsw-alias-label-primary-foreground,#fff);box-shadow:0 4px 16px var(--dpo-err-line)}",
					/* 滚动条：交给宿主滚动条 token，亮暗都有对比 */
					".dpo-overlay-scroll::-webkit-scrollbar-thumb,.dpo-pane::-webkit-scrollbar-thumb,.dpo-pop::-webkit-scrollbar-thumb,.dpo-overlay-body::-webkit-scrollbar-thumb,.dpo-trace::-webkit-scrollbar-thumb,.dpo-review-text::-webkit-scrollbar-thumb{background:var(--dsw-alias-scrollbar-bg-l2,color-mix(in srgb,var(--dsw-alias-label-caption,#777) 45%,transparent))}",
					".dpo-overlay-scroll::-webkit-scrollbar-thumb:hover,.dpo-pane::-webkit-scrollbar-thumb:hover,.dpo-pop::-webkit-scrollbar-thumb:hover{background:var(--dsw-alias-scrollbar-hover-l2,var(--dpo-acc-40))}",
					/* 焦点态：统一可见轮廓，亮暗都认得出 */
					".dpo-review-text:focus{box-shadow:inset 0 1px 2px var(--dpo-lo-soft),0 0 0 3px var(--dpo-acc-12)}",
					".dpo-regen-input:focus{box-shadow:0 0 0 3px var(--dpo-acc-12)}",

					/* ══════════ v53 布局自适应层（追加覆写：只改收缩/换行策略，不动配色与拦截逻辑） ══════════
					   为什么视口断点救不了：上面 v52 的三条断点用的是 @media (max-width:1480/1240/1080px)，
					   看的是**视口**宽度。但真正约束控件的是宿主工具行分给本插件的**可用宽度**
					   （宿主 .uV2eYG_tools 里，本插件与 +/附件/权限选择器并排），而右侧边栏展开只压缩
					   这一份可用宽度、不改视口。实测：DSH Desktop 默认窗口 1721×927 @dpr1.5 → CSS 视口
					   1147px，已落在 ≤1240 分支；此时展开右侧栏，可用宽度继续变小而视口不动，断点不再触发。

					   真正的压坏机制（实测复现）：插件内容的 min-content 约 370px，而可用宽度可以小于它；
					   .dpo-slider 被设了 min-width:0，于是父级可以把它压到比内容更窄（实测 68.8px），
					   而 .dpo-slider-track 的 min-width:76px 不肯让步 ⇒ 轨道越出滑块盒 7.2px，
					   相邻两组轨道间距只剩 1.8px（470px 卡片时还有 31.6px）→ 视觉上挤成一团。

					   修复策略（不依赖任何"测出来的"宿主宽度，任何宿主下都成立）：
					     ① .dpo-slider{overflow:hidden} —— 从机制上杜绝轨道越出滑块盒；
					     ② .dpo-controls 允许换行，让宿主 row 的 flex-wrap 真正生效而不是硬挤一行；
					     ③ 下调各档基准宽 + 把轨道 min-width 从 76px 降到 56px，给收缩留出真余量；
					     ④ 数值胶囊保持不可收缩，宁可轨道短一点也不截断"高级/审查"这类必读文字。

					   为什么不用容器查询：试过给包裹层加 container-type:inline-size，会让该层自身的
					   min-content/max-content 塌成 0（inline-size 包含），实测控件整行宽度直接变 0px，
					   等于把控件弄没了。所以这里改用纯收缩策略，不依赖包含上下文。 */
					/* 收缩策略（第二次修订，依据真实实例实测）：
					   必须让「包裹层」和「控件行」都可收缩，否则外层刚性、内层无处可缩。
					   实测：右侧栏展开后卡片 617.3px、行内容右缘 905.3px；旧写法
					   flex:0 0 auto + min-width:fit-content 让包裹层死停在 541.13px，控件右缘 937.1px
					   ⇒ 越出行内容盒 +31.8px。改为可收缩后收敛到 405.1px、越界 −4.2px。
					   min-width 不能用 fit-content：那等于同时声明"不可收缩 + 最小尺寸=内容宽"。 */
					".dpo-controls-wrap{display:flex;align-items:center;flex:0 1 auto;min-width:0;max-width:100%}",
					/* gap 收成 6px 8px 以换取收缩余量；flex-wrap 让空间不足时换行而不是压叠。
					   注意不要用 .dpo-controls>* {max-width:100%} —— 实测它会把 .dpo-model 的
					   min-width:46px 一起抹掉（max-width 会连 min-width 一起压平），胶囊被压到装不下文字。 */
					".dpo-controls{display:flex;flex-wrap:wrap;flex:0 1 auto;gap:6px 8px;row-gap:6px;max-width:100%;min-width:0}",
					/* 收缩余量：v52 的 min-width:124/76 太大，实测会让轨道撑破被压扁的滑块 7.2px。
					   下限按"任何宿主都不得压坏自身盒"来定，而不是按某个测得的可用宽度去凑：
					   .dpo-slider{overflow:hidden} 保证轨道永不越出滑块盒，
					   track 的 min-width 只决定"还剩下多少可拖动行程"，不参与是否会重叠的判定。
					   实测（300~952px 卡全区间）轨道撑破滑块 = 0，2D 重叠 = 0。 */
					".dpo-slider-track{flex:0 0 120px;min-width:56px}",
					/* 滑块内两件套：数值胶囊保持不可收缩（收一点就会截断"高级/审查"两个字）；
					   溢出兜底裁切，绝不允许压到相邻控件上 */
					".dpo-slider{overflow:hidden}",
					".dpo-slider-value{flex:0 0 auto}",
					/* 1180~1240 视口（DSH Desktop 默认窗口就在这一档）：96px 轨道 + 数值胶囊
					   在卡片 712~952px 全程落在宿主给的可用宽度内（实测余量 −126 ~ −458px） */
					"@media (max-width:1240px){.dpo-slider-track{flex:0 1 96px;min-width:56px}.dpo-slider{gap:6px}}",
					"@media (max-width:1080px){.dpo-slider-track{flex:0 1 84px;min-width:52px}.dpo-controls{gap:8px}}",
					"@media (max-width:940px){.dpo-slider-track{flex:0 1 72px;min-width:48px}.dpo-slider-value{font-size:11.5px;padding:1px 2px}}",

					/* ══════════ v54 挡位条 / 核按钮（模型按钮）重做层 ══════════
					   为什么单开一层而不改上面：--dpo-acc 被 30+ 条规则共用（浮层、通用按钮、弹层条目、
					   徽标、提示条、流式光标），改它会波及本次范围之外的视觉。所以本层只用**独立的
					   --dpo-meter-* 名字空间**，且只作用于两个挡位条（.dpo-slider）与模型按钮（.dpo-model）。

					   形状取舍：原来的"圆头细滑轨 + 发光圆钮 + 小圆点刻度"是音量条语义，既不像挡位、
					   又靠高饱和色撑识别度。这里换成**分段电平表（segmented meter）**：
					     · 轨道按档位数切格，格间 2px 缝 ⇒ "可选范围"一眼可见（tier 4 格 / perm 2 格，
					       格数由同一套样式自动体现，不需要额外标记）
					     · 已选格填 --dpo-meter-ink ⇒ "已选/未选"靠明度差而非色相区分
					     · 当前档位格再叠 2px 内描边，滑块外环同色 ⇒ 当前值可辨
					     · 圆钮保留为拖动把手，改中性墨 + 表面色描边
					   ⇒ 全程无蓝色系，且不依赖背景色相。

					   颜色取舍：一律走 DSH 语义 token 的 color-mix，**本层不出现任何 hex/rgb/hsl 字面量，
					   连 var() 回退值都不给硬编码色**（DSH 恒定提供这些 token；若将来缺失，声明会直接失效而不是
					   退回一个与背景冲突的写死色）。墨色取 --dsw-alias-label-primary
					   （亮色主题=深墨，暗色主题=浅墨），于是"已选"在亮底是深填充、在暗底是浅填充，
					   两种背景下都与底色有足够反差。 */
					".dpo-controls,.dpo-pop{",
					"  --dpo-meter-ink:var(--dsw-alias-label-primary);",
					"  --dpo-meter-ink-08:color-mix(in srgb,var(--dpo-meter-ink) 8%,transparent);",
					"  --dpo-meter-ink-12:color-mix(in srgb,var(--dpo-meter-ink) 12%,transparent);",
					"  --dpo-meter-ink-16:color-mix(in srgb,var(--dpo-meter-ink) 16%,transparent);",
					"  --dpo-meter-ink-40:color-mix(in srgb,var(--dpo-meter-ink) 40%,transparent);",
					/* 表面/填充：目标是"适配任意背景"——用半透明中性色（无色相）叠在背景上，
					   亮底自然变深、暗底自然变亮，因此不依赖具体主题色。
					   未选轨道：12% 墨（很弱）；已选段：46% 墨（明显更强）。
					   两者在任意背景上都能拉开明度差，同时都不会变成"突兀色块"。 */
					"  --dpo-meter-rail:color-mix(in srgb,var(--dpo-meter-ink) 10%,transparent);",
					"  --dpo-meter-frost:color-mix(in srgb,var(--dpo-meter-ink) 52%,transparent);",
					"  --dpo-meter-surf:var(--dsw-alias-bg-layer-1,var(--dpo-surface));",
					/* 轮廓色：单独用 label-tertiary 而不是墨色低透明。
					   原因：底槽若只用 22% 墨做 1px 描边，实测与卡片底的对比只有
					   1.28:1（暗）/ 1.18:1（亮）—— 未填充的格子几乎看不见，"可选范围"就丢了。
					   换成中等权重的三级文字色后，轮廓对比升到可辨区间，且仍是中性色。 */
					"  --dpo-meter-outline:color-mix(in srgb,var(--dsw-alias-label-tertiary) 70%,transparent);",
					/* 核按钮只需更轻的填充量级：它面积小、又是常驻控件，不该和挡位条同重量级。
					   描边沿用上面的 --dpo-meter-outline，保证边界可见。 */
					"  --dpo-model-bg:color-mix(in srgb,var(--dpo-meter-ink) 5%,transparent);",
					"  --dpo-model-bg-hi:color-mix(in srgb,var(--dpo-meter-ink) 10%,transparent);",
					"}",
					/* —— 轨道：分段格子 ——
					   每格宽度固定 30px，轨道总宽 = 格数 × 30px，由组件用内联样式给出
					   （4 档 = 120px，2 档 = 60px ⇒ 审查条正好是等级条的一半，两处每格等大）。
					   这里不再写死宽度：内联优先级高于 v52/v53 的 @media，避免被改成另一套数值而破坏等格。
					   只负责高度与圆角：轨道加高到 24px（视觉轨道靠 ::before 保持 14px 居中），
					   让整条滑轨——而不只是 14px 的细带——都成为可点区域。 */
					".dpo-slider-track{height:24px;border-radius:7px;overflow:hidden}",
					/* 视觉轨道：14px 高、垂直居中，铺满整个可点区域 */
					".dpo-slider-track::before{content:\"\";position:absolute;left:0;right:0;top:5px;height:14px;border-radius:5px;background:var(--dpo-meter-rail);box-shadow:inset 0 0 0 1px var(--dpo-meter-outline);-webkit-backdrop-filter:blur(6px) saturate(1.1);backdrop-filter:blur(6px) saturate(1.1)}",
					/* 已选段：背景色上叠一层半透明磨砂白（按口径），恰好覆盖"已选格数"。
					   z-index:1 让它低于格子层，格上的分隔线才不会被盖住。 */
					".dpo-slider-fill{top:5px;height:14px;border-radius:5px;background:var(--dpo-meter-frost);box-shadow:none;z-index:1;-webkit-backdrop-filter:blur(8px) saturate(1.15);backdrop-filter:blur(8px) saturate(1.15)}",
					/* 档位格：每格一个 .dpo-stop。分隔线用 ::before 画在格子内部，
					   格子 z-index:3 高于 fill 的 1 —— 原先用 box-shadow 画线会被 fill 覆盖，
					   造成"选到某档时上一格的分隔线消失"。 */
					".dpo-stop{position:relative;z-index:3;height:100%;background:transparent;border:none;padding:0;cursor:pointer}",
					".dpo-stop+.dpo-stop::before{content:\"\";position:absolute;left:0;top:5px;bottom:5px;width:2px;margin-left:-1px;background:var(--dpo-meter-surf);pointer-events:none}",
					/* 已选格：1px 内描边（不越出自身，避免压到相邻格的格线） */
					".dpo-stop[data-on=\"true\"]{background:transparent;box-shadow:none}",
					".dpo-stop:disabled{cursor:not-allowed}",
					".dpo-stop:focus-visible{outline:none;box-shadow:inset 0 0 0 2px var(--dpo-meter-ink)}",
					/* 取消原来那个 4px 小圆点刻度（改用格子语义） */
					".dpo-stop::after{content:none}",
					/* 可点范围补足：滑块自身也接收指针（轨道两侧/上下的留白同样可点）。
					   点击落点仍由 pickFromX 按轨道 rect 归一化换算，行为不变。 */
					".dpo-slider{padding:0;overflow:visible;cursor:pointer;touch-action:none}",
					/* —— 把手：按"选到对应档位就把格子填好"的口径，不再显示 —— */
					".dpo-slider-thumb{display:none}",
					/* 键盘可达：焦点环走同一套墨色 */
					".dpo-slider:focus-within .dpo-slider-track{box-shadow:0 0 0 2px var(--dpo-meter-ink-40)}",
					/* —— 数值文字：去掉边框与底色，只留文字；字号/字重/字距与宿主正文汉字完全对齐。
					   对照实测（正文汉字，如模型名）：font-family 同一套、font-weight 400、font-size 12px。
					   原先本处为 600 / 12.5px / letter-spacing .2px，三处都偏"重"。
					   注意还要把 border 显式清零：上面 v52 有 `border:1px solid transparent`，
					   虽不可见，但 computed border-width 仍是 0.667px，属于"残留边框"。 */
					".dpo-slider-value{font-weight:400;font-size:12px;letter-spacing:normal;padding:0;border:none;border-radius:0;background:none;box-shadow:none;text-shadow:none;color:var(--dsw-alias-label-primary)}",
					".dpo-slider[data-disabled=\"true\"] .dpo-slider-value{background:none;border:none;box-shadow:none;color:var(--dsw-alias-label-tertiary)}",
					".dpo-slider[data-disabled=\"true\"]{opacity:.45}",
					/* —— 核按钮（模型按钮）：同一套变量族，但用更轻的量级 —— */
					".dpo-model{border-radius:7px;border:1px solid var(--dpo-meter-outline);background:var(--dpo-model-bg);color:var(--dsw-alias-label-primary);box-shadow:none;transition:background .16s ease,border-color .16s ease,color .16s ease,transform .16s var(--dpo-spring)}",
					".dpo-model:hover{transform:translateY(-1px);border-color:var(--dpo-meter-ink-40);background:var(--dpo-model-bg-hi);color:var(--dsw-alias-label-primary);box-shadow:0 1px 4px var(--dpo-meter-ink-12)}",
					".dpo-model:active{transform:translateY(0) scale(.98);background:var(--dpo-meter-ink-16)}",
					".dpo-model[data-open=\"true\"]{border-color:var(--dpo-meter-ink-40);background:var(--dpo-model-bg-hi);color:var(--dsw-alias-label-primary);box-shadow:0 0 0 3px var(--dpo-meter-ink-12)}",
					".dpo-model:focus-visible{outline:none;box-shadow:0 0 0 3px var(--dpo-meter-ink-40)}",
					/* 弹层内的档位条与工具栏共用同一套变量与尺寸，避免两处观感不一致 */
					".dpo-pop .dpo-slider-track{height:14px}",
				].join("\n");

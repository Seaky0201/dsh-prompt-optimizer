window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/* 对外口径 = 「**本会话**被接管的发送次数」，两个维度都要对：
		   ① 不是 record 条数 —— 一次发送会同时记 keydown-enter/click-send 与 optimize-start 两条，
		      直接用 intercepts.length 会把 1 次发送显示成 2；
		   ② 不是全局 —— intercepts 是跨会话共用的数组，必须按当前会话过滤，
		      否则 A 会话拦过 1 次、切到 B 会话徽标照样显示 1（"按会话独立"是本插件的一贯语义）。 */
		function interceptedSends() {
			const key = store.viewSessionId || null;
			let c = 0;
			for (const r of store.intercepts) {
				if ((r.kind === "keydown-enter" || r.kind === "click-send") && (r.sessionId || null) === key) c++;
			}
			return c;
		}
		...
		function helpPopover() {
			...
			return h("div", {
				...
				h("div", { className: "dpo-help-meta", "data-dpo": "help-meta" },
					"作者：啃轮胎的西狐 · 版本 0.1.1beta4 · 版本日期 2026/09/13"),
			);
		}

...
function sanitizePerSession(raw) {
  ...
  for (const id of Object.keys(raw).slice(-PER_SESSION_CAP)) {
    ...
    // 接管计数（按会话）：非负整数，缺失/非法一律丢弃；上限 1e9 防止脏数据
    if (typeof v.sends === 'number' && Number.isFinite(v.sends) && v.sends > 0) entry.sends = Math.min(1e9, Math.round(v.sends))
...
function savePluginState(patch) {
  ...
  if (sid) {
    ...
    // 接管计数（按会话）：客户端在每次拦截后上报 sends，落盘后跨刷新/重启保留
    if (typeof patch.sends === 'number' && Number.isFinite(patch.sends) && patch.sends > 0) entry.sends = Math.min(1e9, Math.round(patch.sends))
